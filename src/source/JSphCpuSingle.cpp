...
void JSphCpuSingle::RunCellDivide(bool updateperiodic){
  ...
  CellDivSingle->SortArray(Tempc);//temperature
  if(TStep==STEP_Verlet){
    CellDivSingle->SortArray(VelrhopM1c);
	CellDivSingle->SortArray(TempM1c);//temperature
  }
	...
	CellDivSingle->SortArray(TempPrec);//temperature
  }
  ...
  if(npfout){
    ...
    unsigned num=GetParticlesData(npfout,Np,true,false,idp,pos,vel,rhop,temp,code);
...
void JSphCpuSingle::AbortBoundOut(){
  ...
  double* temp = ArraysCpu->ReserveDouble();
  typecode* code=ArraysCpu->ReserveTypeCode();
  GetParticlesData(nboundout,Np,true,false,idp,pos,vel,rhop,temp,code);
...
void JSphCpuSingle::Interaction_Forces(TpInter tinter){
  ...
  float viscdt=0;
  if(Psingle)JSphCpu::InteractionSimple_Forces(Np,Npb,NpbOk,CellDivSingle->GetNcells(),CellDivSingle->GetBeginCell(),CellDivSingle->GetCellDomainMin(),Dcellc,PsPosc,Velrhopc,Tempc,Idpc,Codec,Pressc,viscdt,Arc,Acec, Atempc, Deltac,SpsTauc,SpsGradvelc,ShiftPosc,ShiftDetectc);
  else JSphCpu::Interaction_Forces(Np,Npb,NpbOk,CellDivSingle->GetNcells(),CellDivSingle->GetBeginCell(),CellDivSingle->GetCellDomainMin(),Dcellc,Posc,Velrhopc,Tempc,Idpc,Codec,Pressc,viscdt,Arc,Acec,Atempc,Deltac,SpsTauc,SpsGradvelc,ShiftPosc,ShiftDetectc);
...
void JSphCpuSingle::SaveData(){
  ...
  if(save){
	...
	temp = ArraysCpu->ReserveDouble();//temperature
    unsigned npnormal=GetParticlesData(Np,0,true,PeriActive!=0,idp,pos,vel,rhop,temp,NULL);
  ...
  ArraysCpu->Free(vel);
  ArraysCpu->Free(rhop);
  ArraysCpu->Free(temp);//temperature
